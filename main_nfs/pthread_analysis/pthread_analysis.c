...
float dota_atof (char unitl) //胡磊2020.07.16：函数作用：把字符串转化为浮点
{
	if (unitl > 100)
	{
		return unitl / 1000.0;
...
int dota_atoi (const char * cDecade) //胡磊2020.07.16：函数作用：把字符串转化为整型数
{
	int result = 0;
	if (' ' != cDecade[0])
	{
...
int checkEnv (int sto_no, struct storage_info *p)
{
	...
	if (0 == tem_alarm_status[sto_no]) //胡磊2020.07.23：温度超标
	{
		if (p->temperature > p->temperatureMAX) //胡磊2020.07.23：如果温度高于设定的最高温度则报警
		 {
			sendMsgQueue (MSG_LED, MSG_LED_TEM_ON);     //fs11c14上的灯
			sendMsgQueue (MSG_M0, MSG_M0_LED_ON | sto_no << 6);//MSG_M0 transfer to M0
			sendMsgQueue (MSG_M0, MSG_M0_FAN_ON3 | sto_no << 6); //胡磊2020.07.23：为什么左移6位？
			p->fan_status = 3;
			tem_alarm_status[sto_no] = 2;
			flag = 1;
		}
		else if (p->temperature < p->temperatureMIN)//胡磊2020.07.23：如果温度低于设定的最低温度则报警，设置和最高温度报警不一样？
		{
			sendMsgQueue (MSG_LED, MSG_LED_TEM_ON);     //fs11c14上的灯
		...
		if (flag)
		{	
			pthread_mutex_lock (&mutex_sms);//胡磊2020.07.23：mutex_sms是短信模块互斥锁
			if (sto_no == 1)
				dev_sms_cmd = SMS_TEM |SMS_STRNO1;
			else
				dev_sms_cmd = SMS_TEM |SMS_STRNO2;
			pthread_mutex_unlock (&mutex_sms);
			pthread_cond_signal (&cond_sms); //胡磊2020.07.23：唤醒短信模块线程
			flag = 0;
	#if 1
			if (beep_status[sto_no] == 0) //胡磊2020.07.23：蜂鸣器状态为0
			{
				printf("*****345beep*****");
				beep_status[sto_no] = 1;
				p->buzzer_status = 1;	    //9-16 胡磊2020.07.23：为什么写了两遍？
	...
	{
		if (p->temperature < p->temperatureMAX && p->temperature > p->temperatureMIN) //胡磊2020.07.23：温度正常
		{
	...
	if (0 == hum_alarm_status[sto_no])
	{
			...
			if (p->humidity > p->humidityMAX) //胡磊2020.07.23：如果湿度高于设定的最高湿度则报警
		{
		...
		else if (p->humidity < p->humidityMIN) //胡磊2020.07.23：如果湿度低于设定的最低湿度则报警
		{
			sendMsgQueue (MSG_LED, MSG_LED_HUM_ON);
			sendMsgQueue (MSG_M0, MSG_M0_LED_ON | sto_no << 6);//MSG_M0 transfer to M0
			sendMsgQueue (MSG_M0, MSG_M0_FAN_OFF | sto_no << 6);
	...
	if (0 == ill_alarm_status[sto_no])
	{
		if(p->illumination !=0)
		{
				if (p->illumination > p->illuminationMAX) //胡磊2020.07.23：如果光照大于最高光照
				{
...
void getGoodsPackage (link_datatype *buf) //胡磊2020.07.23：增减货物？
{
